...
public class RoomController {
    ...
    // ─── Create Room ──────────────────────────────────────────────────────────
    // POST /api/v1/rooms?roomname=MyRoom
    // Body: plain-text roomId
    @PostMapping
    ...
    @GetMapping("/{roomId}/roomname")
    public ResponseEntity<?> getRoomName(@PathVariable String roomId) {
        Room room = roomRepository.findByRoomId(roomId);
        if (room == null) {
            return ResponseEntity.badRequest().body("Room not found!!");
        }
        return ResponseEntity.ok(room.getRoomname());
    }
